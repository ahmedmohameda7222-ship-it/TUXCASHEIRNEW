...
describe('PublicCatalogSnapshotV1', () => {
  ...
  it('rejects unknown schema versions', () => {
    expect(() =>
      parsePublicCatalogSnapshotV1({ ...validSnapshot(), schemaVersion: 2 }),
  ...
  it('rejects missing required fields', () => {
    ...
    delete product.slug;
    expect(() =>
      parsePublicCatalogSnapshotV1({ ...snapshot, products: [product] }),
    ).toThrow(/slug/);
  });
...
describe('CatalogErrorV1', () => {
  it('parses a stable public error', () => {
    expect(
      parseCatalogErrorV1({
        schemaVersion: 1,
        error: { code: 'catalog_read_failed' },
      }),
