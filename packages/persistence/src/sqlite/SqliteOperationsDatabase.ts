  ...
  return {
    ...
    devices: {
      async getById(id: DeviceId) {
        return parsePayload<Device>(database.prepare('SELECT payload_json FROM devices WHERE id = ?').get(id));
      },
    ...
    workers: {
      ...
      async put(worker: Worker) {
        database
          ...
          .run(worker.id, worker.shopId, worker.displayName, worker.pinHash, worker.active ? 1 : 0, serialize(worker));
      },
    },
    workerSessions: {
      async put(session: WorkerSession) {
        database
          ...
          .run(session.id, session.shopId, session.businessDayId, session.workerId, session.startedAt, session.endedAt, serialize(session));
      },
    },
    configuration: {
      async getForShop(shopId: ShopId) {
        return parsePayload<OperationsConfigurationSnapshot>(
          database.prepare('SELECT payload_json FROM configuration_snapshots WHERE shop_id = ?').get(shopId),
        );
      },
    ...
    customerContacts: {
      async getByNormalizedPhone(shopId: ShopId, normalizedPhone: string) {
        return parsePayload<CustomerContact>(
          database
            .prepare('SELECT payload_json FROM customer_contacts WHERE shop_id = ? AND normalized_phone = ?')
      ...
      async put(contact: CustomerContact) {
        database
          ...
          .run(contact.id, contact.shopId, contact.normalizedPhone, contact.displayPhone, contact.name, contact.lastOrderAt, serialize(contact));
      },
    },
    businessDays: {
      async getById(id: BusinessDayId) {
        return parsePayload<BusinessDay>(database.prepare('SELECT payload_json FROM business_days WHERE id = ?').get(id));
      },
      async getOpenForShop(shopId: ShopId) {
        return parsePayload<BusinessDay>(
          database.prepare("SELECT payload_json FROM business_days WHERE shop_id = ? AND status = 'OPEN' LIMIT 1").get(shopId),
        );
      },
      async put(day: BusinessDay) {
        database
          ...
          .run(day.id, day.shopId, day.status, day.startedAt, day.endedAt, day.startedByWorkerId, day.endedByWorkerId, day.lastAllocatedDisplayOrderNo, serialize(day));
      },
    },
    orders: {
      ...
      async getByIdempotencyKey(shopId: ShopId, idempotencyKey: string) {
        return parsePayload<OrderSnapshot>(
          database.prepare('SELECT payload_json FROM orders WHERE shop_id = ? AND idempotency_key = ?').get(shopId, idempotencyKey),
        );
      },
      async listByBusinessDay(businessDayId: BusinessDayId) {
        return database
          .prepare('SELECT payload_json FROM orders WHERE business_day_id = ? ORDER BY created_at ASC, display_order_no ASC')
          .all(businessDayId)
      ...
      async insert(order: OrderSnapshot) {
        database
          ...
          .run(order.id, order.shopId, order.businessDayId, order.displayOrderNo, order.idempotencyKey, order.status, order.source, order.operatorWorkerId, order.createdAt, order.totalMinor, serialize(order));
      },
      async updateOperationalState(order: OrderSnapshot) {
        ...
        database.prepare('UPDATE orders SET status = ?, payload_json = ? WHERE id = ?').run(updated.status, serialize(updated), updated.id);
      },
    },
    expenses: {
      async put(expense: Expense) {
        database
          ...
          .run(expense.id, expense.shopId, expense.businessDayId, expense.kind, expense.amountMinor, expense.paidFrom, expense.orderId, expense.createdAt, serialize(expense));
      },
    },
    inventory: {
      ...
      async appendMovement(movement: InventoryMovement) {
        database
          ...
          .run(movement.id, movement.shopId, movement.businessDayId, movement.itemId, movement.movementType, movement.quantityDeltaMicros, movement.idempotencyKey, movement.workerId, movement.orderId, movement.createdAt, movement.compensatesMovementId, serialize(movement));
      },
      async listMovementsForOrder(orderId: OrderId) {
        return database
          .prepare('SELECT payload_json FROM inventory_movements WHERE order_id = ? ORDER BY created_at ASC, id ASC')
          .all(orderId)
    ...
    reconciliations: {
      async put(reconciliation: Reconciliation) {
        database
          ...
          .run(reconciliation.id, reconciliation.shopId, reconciliation.businessDayId, reconciliation.createdByWorkerId, reconciliation.createdAt, serialize(reconciliation));
      },
    },
    audit: {
      async append(event: AuditEvent) {
        database
          ...
          .run(event.id, event.shopId, event.businessDayId, event.aggregateType, event.aggregateId, event.eventType, event.workerId, event.createdAt, serialize(event));
      },
    },
    outbox: {
      async append(event: OutboxEvent) {
        database
          ...
          .run(event.id, event.shopId, event.businessDayId, event.aggregateType, event.aggregateId, event.eventType, event.idempotencyKey, event.payloadVersion, serialize(event), event.createdAt, event.attemptCount, event.nextAttemptAt, event.lastError, event.deliveredAt);
      },
      ...
      async markDelivered(id: OutboxEventId, deliveredAt: Instant) {
        const existing = parsePayload<OutboxEvent>(database.prepare('SELECT payload_json FROM outbox_events WHERE id = ?').get(id));
      ...
      async quarantine(id: OutboxEventId, quarantinedAt: Instant, reason: string) {
        const existing = database.prepare('SELECT id FROM outbox_events WHERE id = ?').get(id);
        if (existing === undefined) throw new Error(`Outbox event ${id} was not found.`);
        database
          .prepare(
            `UPDATE outbox_events
             SET quarantined_at = ?, permanent_failure_reason = ?, last_error = ?, next_attempt_at = NULL
             WHERE id = ?`,
          )
          .run(quarantinedAt, reason, reason, id);
      },
    },
  };
}
