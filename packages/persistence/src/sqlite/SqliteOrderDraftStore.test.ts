...
import { SqliteOrderDraftStore } from './SqliteOrderDraftStore';
...
const dirs: string[] = [];
function path() {
  const dir = mkdtempSync(join(tmpdir(), 'tux-draft-'));
  dirs.push(dir);
  return join(dir, 'drafts.sqlite');
}
...
describe('SqliteOrderDraftStore parked drafts', () => {
  it('atomically parks the active draft and replaces it, then survives restart', async () => {
    const file = path();
    const store = new SqliteOrderDraftStore(file);
    ...
    await store.close();
    const reopened = new SqliteOrderDraftStore(file);
  ...
  it('rejects a stale revision without mutation', async () => {
    const store = new SqliteOrderDraftStore(path());
    await store.initialize();
  ...
  it('restores a parked draft and optionally parks current active', async () => {
    const store = new SqliteOrderDraftStore(path());
    await store.initialize();
    ...
    const result = await store.restoreParked({
      ...
      parkedId: 'parked-1',
      parkActiveAs: parked(current, 'parked-current'),
  ...
  it('discards explicitly and hides resolved history', async () => {
    const store = new SqliteOrderDraftStore(path());
    await store.initialize();
    const active = draft(2, '44444444-4444-4444-8444-444444444444');
    await store.put(active);
  ...
  it('rejects cross-shop/day restore and discard', async () => {
    const store = new SqliteOrderDraftStore(path());
    await store.initialize();
    const active = draft(2, '44444444-4444-4444-8444-444444444444');
    await store.put(active);
