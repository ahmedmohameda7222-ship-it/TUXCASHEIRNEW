...
function createRemote(): WhatsAppRemoteGateway {
  return {
    ...
    sendMedia: vi.fn().mockRejectedValue(new Error('not called')),
    sendLocation: vi.fn().mockRejectedValue(new Error('not called')),
    sendTemplate: vi.fn().mockRejectedValue(new Error('not called')),
    retryFailedMessage: vi.fn().mockRejectedValue(new Error('not called')),
    getMediaAccess: vi.fn().mockRejectedValue(new Error('not called')),
