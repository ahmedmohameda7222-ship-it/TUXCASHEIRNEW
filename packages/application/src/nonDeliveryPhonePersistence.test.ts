...
const modelsSource = readFileSync(
  new URL('../../domain/src/models.ts', import.meta.url),
  'utf8',
);
...
describe('required non-delivery customer phone persistence', () => {
  it(
    'keeps validated pickup/take-away phone authority in the immutable fulfillment snapshot',
    () => {
      ...
      expect(ordersSource).toContain(
        'normalizedPhone: input.normalizedDeliveryPhone',
      );
