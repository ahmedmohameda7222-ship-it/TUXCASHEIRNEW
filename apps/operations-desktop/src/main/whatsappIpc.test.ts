...
function service(): TuxWhatsAppApi {
  return {
    loadInbox: vi.fn(async () => ({ ok: true, value: { conversations: [], messages: [], quickReplies: [], orderLinks: [], nextCursor: null } })) as never,
    loadConversation: vi.fn(async () => ({ ok: true, value: [] })) as never,
    sendText: vi.fn(async () => ({ ok: false, error: { code: 'REMOTE_SYNC_ERROR', message: 'not used' } })) as never,
