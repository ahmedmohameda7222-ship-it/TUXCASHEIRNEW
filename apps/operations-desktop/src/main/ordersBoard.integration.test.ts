...
describe('Operations Orders Board SQLite integration', () => {
  ...
  it(
    'converts an active reservation into costed consumption when prepared food is cancelled',
    async () => {
      const fx = await fixture();
      ...
      expect(
        movements.some((movement) => movement.movementType === 'ORDER_RESERVATION_RELEASE'),
      ).toBe(false);
