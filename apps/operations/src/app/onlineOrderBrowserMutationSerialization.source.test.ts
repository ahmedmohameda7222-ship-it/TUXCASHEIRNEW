...
describe('browser online-order mutation serialization source', () => {
  it(
    'holds one browser request lock through acceptance placement and durable tombstoning',
    () => {
      const panel = readFileSync(
        new URL('./OnlineOrderInboxPanel.tsx', import.meta.url),
        'utf8',
      );
      ...
      const lockStart = acceptSource.indexOf(
        'browserOnlineOrderMutations.run(request.requestId',
      );
