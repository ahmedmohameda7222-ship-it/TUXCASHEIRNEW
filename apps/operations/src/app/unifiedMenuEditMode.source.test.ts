...
describe('unified menu edit entry point', () => {
  ...
  it('uses dnd-kit spatial keyboard sorting and keeps immediate drops tied to the stock geometric target', () => {
    expect(ordersWorkspaceSource).toContain('KeyboardSensor');
  ...
  it('resets the product drag sensor when a category change invalidates an in-flight product pickup', () => {
    expect(ordersWorkspaceSource).toContain("key={selectedCategoryId ?? 'menu-products-none'}");
    expect(ordersWorkspaceSource).toContain("dispatchMenuLayoutEditor({ type: 'CATEGORY_CHANGE' })");
  });

  it('keeps idle reorder hit targets geometrically stable while preserving the legacy jiggle cue, a clear active affordance, and Reduced Motion', () => {
