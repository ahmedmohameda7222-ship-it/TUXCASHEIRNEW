...
describe('persisted combo checkout authority', () => {
  it('blocks a persisted combo when the current catalog has no eligible beverage left', () => {
    ...
    expect(source).toMatch(/comboBeveragesByProduct\s*,\s*products|products\s*,\s*comboBeveragesByProduct/);
    expect(source).toMatch(/products\.find\(\(product\)\s*=>\s*product\.id\s*===\s*productId\)/);
