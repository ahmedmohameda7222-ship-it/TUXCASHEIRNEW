...
describe('CRM store canonical customer lineage', () => {
  it(
    'shows retired loyalty history while using an unpaginated canonical balance',
    async () => {
      ...
      const ledgerCall = select.mock.calls.find(
        ([table]) => table === 'loyalty_ledger',
      );
      ...
      expect(rpc).toHaveBeenCalledWith(
        'get_admin_customer_loyalty_balance_v1',
        {
  ...
  it(
    'pages all shop links before applying an exact customer search',
    async () => {
      ...
      const select = vi.fn(async (table: string, query: URLSearchParams) => {
        ...
        if (table === 'business_customers') {
          ...
          return ids.includes(targetId) &&
            search.includes('Hidden exact customer')
            ? [
                {
                  id: targetId,
      ...
      const listLinkCalls = select.mock.calls.filter(
        ([table, query]) =>
          table === 'customer_shop_links' &&
          !query.get('canonical_customer_id'),
      );
