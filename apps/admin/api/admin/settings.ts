...
const weeklyExpectedRowSchema = z
  .object({
...
const specialExpectedRowSchema = z
  .object({
    ...
    .superRefine((command, context) => {
      if ((command.hoursId === null) !== (command.expectedRow === null)) {
        context.addIssue({
          code: 'custom',
          path: ['expectedRow'],
          message: 'Weekly-hours row fence is inconsistent.',
        });
      }
    ...
    .superRefine((command, context) => {
      if ((command.hoursId === null) !== (command.expectedRow === null)) {
        context.addIssue({
          code: 'custom',
          path: ['expectedRow'],
          message: 'Special-hours row fence is inconsistent.',
        });
      }
      if (command.hoursId === null && !command.active) {
        context.addIssue({
          code: 'custom',
          path: ['active'],
          message: 'A new special-hours row must be active.',
        });
      }
      if (
        command.active &&
        command.closed &&
        (command.opensLocal !== null || command.closesLocal !== null)
      ) {
        context.addIssue({
          code: 'custom',
          path: ['opensLocal'],
          message: 'Closed special dates cannot have service times.',
        });
      }
      if (
        command.active &&
        !command.closed &&
        (command.opensLocal === null ||
          command.closesLocal === null ||
          command.opensLocal === command.closesLocal)
      ) {
        context.addIssue({
          code: 'custom',
          path: ['opensLocal'],
          message: 'Open special dates require distinct opening and closing times.',
        });
      }
    }),
  ...
  try {
    ...
    switch (command.type) {
      ...
      case 'shop.identity.update':
        sendJson(response, 200, { ...(await updateShopIdentity(client, command, principal)) });
        return;
      case 'shop.weekly-hours.upsert':
        sendJson(response, 200, { ...(await upsertShopWeeklyHours(client, command, principal)) });
        return;
      case 'shop.special-hours.upsert':
        sendJson(response, 200, { ...(await upsertShopSpecialHours(client, command, principal)) });
        return;
