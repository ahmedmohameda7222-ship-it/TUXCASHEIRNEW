...
test('desktop pointer and spatial keyboard reorder persist after save and re-entry', async ({
  page,
}, testInfo) => {
  ...
  await attachMenuLayoutScreenshot(page, testInfo, 'desktop-menu-drag');
  await page.mouse.up();
  cards = menuEditProductCards(page);
  await expect(cards.nth(1)).toContainText('Triple Smashed Patty');
