...
Operations remains live execution authority, Menu remains customer-facing authority, and canonical Supabase remains the only business source of truth. Browser code must never receive service-role/provider secrets. Admin mutations are network-only and server-authorized. The Admin repository contract permits main-only deployments, but the Admin Vercel project must not be created until the existing Operations project is cut over to Root Directory `apps/operations` and the legacy root `/vercel.json` is removed after successful production verification. PR and feature-branch previews remain disabled. Final production acceptance remains gated by Plan 10, so a pre-Plan-10 Admin deployment is not by itself an accepted production release.
...
The initial Admin Vercel config was too thin for a workspace-based monorepo. It was hardened before Plan 1 acceptance to install and build from the repository root and preserve filesystem/API functions before the SPA fallback. The Admin target contract is rooted at `apps/admin`, but project creation is intentionally gated while the live Operations project still depends on the legacy repository-root `/vercel.json`. Operations must first be cut over to Root Directory `apps/operations`, outside-root source access must be enabled, and the app-local Operations production deployment must be verified before the root contract is removed. Only after that cleanup may the separate Admin Vercel project be created. The Admin app-local contract still permits Git deployment from `main` only while PR and feature-branch previews remain disabled. The Admin security/deployment gates enforce those invariants. Final production acceptance remains gated by Plan 10.
