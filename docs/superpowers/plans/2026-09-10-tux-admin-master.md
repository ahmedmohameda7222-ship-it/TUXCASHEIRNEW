...
`2026-09-10-tux-admin-review-hardening-3.md` is mandatory and closes the next cross-app/deployment gaps found by exact-head review: every scheduled Admin route must have an authenticated Vercel cron trigger and tested cadence; supplier-payment allocations and purchase returns must serialize against canonical PO payable state; receipt identity/order-number settings must propagate into Operations numbering and immutable printer snapshots; and central reason codes must propagate into Operations and be enforced/snapshotted at every applicable mutation boundary.

`2026-09-10-tux-admin-review-hardening-4.md` is mandatory and closes the latest exact-head P1 gaps: PIN-change approvals must persist no raw credential input and audit must contain no credential secret material; canonical POS/ONLINE payments and refunds must project exactly once into mapped finance accounts; the root WhatsApp event dispatcher must have an authenticated production cron; Admin cancellations must proactively flow into idle online Operations clients through a durable lifecycle feed; and globally limited POS promotions/loyalty redemption must acquire an idempotent online reservation before local finalization.
...
- [ ] **Gate 3: Approval/audit** — sensitive commands can require recent acting-user re-PIN and/or be held for approval; second-person rules reject requester self-approval; approval/rejection with a distinct authorized approver executes at most once, remains recoverable across timeout/crash/retry, and a production runner independently drains READY/RETRYABLE/expired-lease jobs; PIN-change approvals persist no plaintext PIN and audit/execution metadata contains no credential verifier/lookup secret; immutable audit history is appended.
...
- [ ] **Gate 5: Orders/CRM/delivery** — Admin cancellation and Operations lifecycle sync converge through canonical compare-and-swap revision authority **and** a proactive authenticated lifecycle feed so an idle online POS receives a remote cancellation without first emitting a stale event; stale local outbox transitions cannot overwrite the canonical winner; order history remains immutable; refund events are separate; applicable cancellation/refund/discount/comp reasons use central code-plus-label snapshots rather than free-text classification; canonical phone identity and merge preserve history; delivery routing is shop-safe; loyalty/promotion rules are enforced atomically in POS and ONLINE order placement with immutable applied-rule snapshots; globally limited POS rewards/loyalty redemption require an idempotent online reservation and fail closed while offline.
...
- [ ] **Gate 7: Finance/reports** — the existing finance core is extended without recreation; canonical POS/ONLINE payments and refunds map/project exactly once into tracked accounts with stable source identity and explicit reconciliation gaps for unmapped/unprojected sources; Operations remains the sole authority that closes canonical business days; Admin X is non-closing and Admin Z/history finalizes exactly once only from an already CLOSED Operations business day; non-zero cash variance/pay-in/pay-out use central structured reason snapshots; profit and money position are separately correct; staff salary expense is included exactly once; settlements do not duplicate sale inflows; transfers do not become expenses; private expense receipts remain access-controlled; reports reconcile to source records.
- [ ] **Gate 8: WhatsApp/operations health** — WhatsApp control surfaces use the existing WhatsApp authority, live replies remain Operations-owned, system-event messages are idempotent and the root backend Vercel deployment schedules their authenticated dispatcher independently of user traffic, domain alerts are deduplicated/actionable, and device/shop health exposes no dangerous remote POS commands.
...
- [ ] **Gate 10: Production** — root CI, migration tests, Menu/Operations/Admin E2E, accessibility gate, mobile Safari/Chrome acceptance, PWA install, exact separate-Vercel deployment-contract checks covering **every** `apps/admin/api/cron/*.ts` route/path/cadence/authentication contract **and** the root `/api/whatsapp-event-dispatch` cron while preserving media retention, payment/finance reconciliation, lifecycle-feed reconnect tests, limited-reward reservation races, and real production smoke checks pass with no unresolved serious review finding.
...
At the final production gate also run the Admin-specific architecture, cross-app, deployment-contract, finance-reconciliation, lifecycle-feed, reward-reservation, and accessibility checks introduced by the reliability/hardening plans.
