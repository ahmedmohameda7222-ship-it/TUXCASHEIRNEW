...
function store(overrides: Partial<PublicCatalogStore> = {}): PublicCatalogStore {
  return {
    ...
    listCategories: async () => [
      { id: CATEGORY_B, shop_id: SHOP_ID, slug: 'fries', name: 'Fries', description: null, active: true, sort_order: 2, internal_note: 'never expose' },
      { id: CATEGORY_A, shop_id: SHOP_ID, slug: 'burgers', name: 'Burgers', description: 'Smashed burgers', active: true, sort_order: 1 },
    ],
    listProducts: async () => [
      { id: PRODUCT_B, shop_id: SHOP_ID, category_id: CATEGORY_A, slug: 'double', name: 'Double', description: null, price_minor: 24000, image_key: null, best_seller: false, active: false, sold_out: true, is_combo: false, sort_order: 2, cost_minor: 1 },
      { id: PRODUCT_A, shop_id: SHOP_ID, category_id: CATEGORY_A, slug: 'single', name: 'Single', description: 'Single burger', price_minor: 19000, image_key: `${SHOP_ID}/aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa.webp`, best_seller: true, active: true, sold_out: false, is_combo: false, sort_order: 1 },
    ],
    listModifiers: async () => [
    ...
    listProductModifierLinks: async () => [
      { shop_id: SHOP_ID, product_id: PRODUCT_A, modifier_id: MODIFIER_ID, max_quantity: 2, sort_order: 1 },
    ],
...
Deno.test('catalog-public rejects invalid shop UUID', async () => {
  ...
  assert(((await body(response)).error as { code: string }).code === 'invalid_shop_id', 'wrong code');
});
...
Deno.test('catalog-public distinguishes unknown shop from empty catalog', async () => {
  ...
  assert(((await body(response)).error as { code: string }).code === 'shop_not_found', 'wrong code');
});
...
Deno.test('catalog-public returns a legitimate empty catalog for an existing shop', async () => {
  ...
  assert(Array.isArray(payload.categories) && payload.categories.length === 0, 'categories not empty');
...
Deno.test('catalog-public backend failure is not converted to an empty catalog', async () => {
  const response = await handleCatalogPublicRequest(
    request(),
    store({ listProducts: async () => { throw new Error('database secret detail'); } }),
  );
  const payload = await body(response);
...
Deno.test('catalog-public fails closed if a store returns a cross-shop row', async () => {
  const response = await handleCatalogPublicRequest(
    ...
    store({
      listProducts: async () => [
        { id: PRODUCT_A, shop_id: OTHER_SHOP_ID, category_id: CATEGORY_A, slug: 'single', name: 'Single', description: null, price_minor: 19000, image_key: null, best_seller: false, active: true, sold_out: false, is_combo: false, sort_order: 1 },
      ],
    }),
  );
...
Deno.test('catalog-public emits deterministic ordering and customer-safe fields only', async () => {
  ...
  assert(products[0]?.id === PRODUCT_A && products[1]?.id === PRODUCT_B, 'product ordering unstable');
...
Deno.test('catalog-public revision is stable for equivalent projections and changes for public content', async () => {
  ...
  const changed = await body(await handleCatalogPublicRequest(
    request(),
    store({
      listProducts: async () => [
        { id: PRODUCT_A, shop_id: SHOP_ID, category_id: CATEGORY_A, slug: 'single', name: 'Single changed', description: 'Single burger', price_minor: 19000, image_key: null, best_seller: true, active: true, sold_out: false, is_combo: false, sort_order: 1 },
      ],
    }),
  ));
