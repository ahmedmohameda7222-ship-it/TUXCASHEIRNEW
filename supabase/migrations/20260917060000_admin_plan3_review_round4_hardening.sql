...
-- Additive Plan 3 hardening for final review round 4. Do not edit already-applied Plan 3 migrations.
...
begin
  ...
  if v_value ~ '(^|[^a-z0-9])(pin|passcode)([[:space:][:punct:]]{1,8}((is|to)[[:space:][:punct:]]{1,8})?)([0-9]([[:space:]-]?[0-9]){3,11})([^0-9]|$)' then
    return true;
  end if;
