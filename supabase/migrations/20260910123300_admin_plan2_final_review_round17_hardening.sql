-- Keep public fulfillment choices aligned with trusted order-intake service-hour authority.
-- Rollout compatibility is preserved per service kind: no active weekly/special hours means open.

create or replace function private.catalog_public_service_kind_open_v1(
  p_settings jsonb,
  p_service_kind text,
  p_now timestamptz
)
...
begin
  if jsonb_typeof(p_settings) <> 'object'
     or p_now is null
     or p_service_kind not in ('OPEN', 'DELIVERY', 'ONLINE') then
    return false;
  end if;
  ...
  select exists (
    ...
    where jsonb_typeof(hours) = 'object'
      and hours ->> 'serviceKind' = p_service_kind
  ...
  if not v_has_weekly and not v_has_special then
    return true;
  end if;
  ...
  where jsonb_typeof(candidate.hours) = 'object'
    and candidate.hours ->> 'serviceKind' = p_service_kind
...
set search_path = pg_catalog, private
as $$
  select private.catalog_public_service_kind_open_v1(p_settings, 'ONLINE', p_now);
$$;
...
begin
  ...
  if v_has_pickup
     and private.catalog_public_service_kind_open_v1(v_settings, 'OPEN', v_now) then
    v_fulfillment_preferences := v_fulfillment_preferences || jsonb_build_array('PICKUP');
  end if;
  if v_has_delivery
     and private.catalog_public_service_kind_open_v1(v_settings, 'DELIVERY', v_now) then
  ...
  return jsonb_build_object(
    ...
    'ordering', jsonb_build_object(
      'available',
        ...
        and private.catalog_public_online_ordering_open_v1(v_settings, v_now),
...
revoke all on function private.catalog_public_service_kind_open_v1(jsonb, text, timestamptz) from public;
...
begin
  if to_regrole('anon') is not null then
    revoke all on function private.catalog_public_service_kind_open_v1(jsonb, text, timestamptz) from anon;
    grant execute on function public.read_catalog_public_ordering_v2(uuid) to anon;
  end if;
  if to_regrole('authenticated') is not null then
    revoke all on function private.catalog_public_service_kind_open_v1(jsonb, text, timestamptz) from authenticated;
