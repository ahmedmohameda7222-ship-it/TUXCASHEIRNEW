-- TUX Admin Plan 2 closure hardening.
-- Repository migration only. Do not apply to a remote project without explicit target authorization.
--
-- 1) Business defaults are business-wide authority and therefore require OWNER/ADMIN,
--    even when a shop-scoped employee has an explicit settings.manage grant.
-- 2) Keep the reviewed setting validator in lockstep with the Plan 2 checkout contract.
...
begin
  ...
  case p_setting_key
    ...
    when 'checkout.requireCustomerPhone',
         ...
         'checkout.allowDiscountStacking' then
...
create or replace function private.assert_admin_business_settings_permission_v1(
  p_employee_id uuid,
  p_shop_id uuid,
  p_permission text
)
...
declare
  ...
  v_denial_code text;
begin
  select a.authorized, a.business_id, a.employee_role, a.denial_code
    into v_authorized, v_business_id, v_employee_role, v_denial_code
  from public.resolve_admin_authorization_v1(p_employee_id, p_shop_id, p_permission) a;

  if not coalesce(v_authorized, false) then
    raise exception 'TUX_ADMIN_PERMISSION_DENIED:%', coalesce(v_denial_code, 'permission_denied');
  end if;

  if v_employee_role not in ('OWNER', 'ADMIN') then
    raise exception 'TUX_ADMIN_BUSINESS_WIDE_PERMISSION_REQUIRED';
  end if;
...
revoke all on function private.assert_admin_business_settings_permission_v1(uuid, uuid, text)
  from public, anon, authenticated;
grant execute on function private.assert_admin_business_settings_permission_v1(uuid, uuid, text)
  to service_role;
...
begin
  ...
  v_business_id := private.assert_admin_business_settings_permission_v1(
