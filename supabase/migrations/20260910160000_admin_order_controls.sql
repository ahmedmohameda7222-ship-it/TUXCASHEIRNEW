...
-- Plan 5 hardening: order lifecycle mutations use compare-and-swap instead of a
-- monotonic silent no-op. Stale Operations events surface the canonical winner.
...
begin
  ...
  if v_existing_found and v_guard ->> 'kind' = 'COMPARE_AND_SWAP_REVISION' then
    if v_guard ->> 'column' <> 'operational_revision'
       or v_guard ->> 'statusColumn' <> 'status'
       or (v_guard ->> 'incomingRevision')::bigint
          <> (v_guard ->> 'expectedRevision')::bigint + 1 then
      raise exception 'TUX_SYNC_GUARD_INVALID';
    end if;
    if coalesce((v_existing ->> 'operational_revision')::bigint, -1)
         <> (v_guard ->> 'expectedRevision')::bigint
       or coalesce(v_existing ->> 'status', '')
         <> coalesce(v_guard ->> 'expectedStatus', '') then
      raise exception 'TUX_ORDER_STALE_OPERATIONAL_REVISION:%:%',
        coalesce(v_existing ->> 'status', '<unknown>'),
        coalesce(v_existing ->> 'operational_revision', '-1');
    end if;
  elsif v_existing_found and not private.tux_guard_allows(v_existing, v_guard) then
    return;
  end if;
...
revoke all on function private.apply_tux_remote_mutation(jsonb)
  from public, anon, authenticated;
