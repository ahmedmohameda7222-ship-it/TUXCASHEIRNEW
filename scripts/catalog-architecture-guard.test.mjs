...
test('rejects Operations coupling, public service-role access, persistence-shaped transport contracts, and second migration authority', async () => {
  const root = await fixture({
    ...valid,
    'packages/catalog-contracts/src/index.ts': "import '@tux/operations'; export interface Row { price_minor: number }\n",
    'apps/menu/src/newClient.ts': "import '@tux/operations';\nclient.from('products').update({ active: false });\n",
  ...
  try {
    ...
    assert.equal(violations.some((value) => value.includes('direct browser catalog mutation')), true);
    assert.equal(violations.some((value) => value.includes('second executable Supabase migration authority')), true);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
...
test('rejects reactivated legacy executable SQL', async () => {
  const root = await fixture({ ...valid, 'apps/menu/supabase_setup.sql': '-- executable legacy authority' });
  try {
    const violations = await collectCatalogArchitectureViolations(root);
    assert.equal(violations.some((value) => value.includes('legacy executable catalog SQL returned')), true);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
