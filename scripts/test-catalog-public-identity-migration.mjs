...
assert.match(sql, /c5579c9a-b2f2-5aa2-b1ed-a3a9b2492b46/i, 'migration must hard-fence the canonical shop');
assert.match(sql, /category_count[^;]*7|count\(\*\)[^;]*<>\s*7/is, 'migration must fence exactly 7 categories');
assert.match(sql, /product_count[^;]*49|count\(\*\)[^;]*<>\s*49/is, 'migration must fence exactly 49 products');
assert.match(sql, /conflicting[^;]*slug|slug[^;]*conflict/is, 'migration must reject conflicting pre-existing slugs');
assert.match(sql, /inventory[^;]*(?:mismatch|drift)|uuid[^;]*(?:set|inventory)[^;]*(?:mismatch|drift)/is, 'migration must fail closed on inventory drift');
assert.match(sql, /post-update|post update|verification failed/is, 'migration must verify the final bindings');

const updateStatements = [...sql.matchAll(/update\s+public\.(menu_categories|products)[\s\S]*?;/gi)].map(
  (match) => match[0],
);
assert.equal(updateStatements.length, 2, 'migration must contain exactly two catalog UPDATE statements');
for (const statement of updateStatements) {
  assert.match(statement, /set\s+slug\s*=/i, 'catalog UPDATE must assign slug');
  assert.doesNotMatch(
    statement,
