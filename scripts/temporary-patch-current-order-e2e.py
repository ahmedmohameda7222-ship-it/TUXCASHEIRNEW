...
old_mobile = """  const payment = overlay.locator('.payment-section');
...
new_mobile = """  const payment = overlay.locator('.payment-section');
...
old_emphasis = """  for (const name of ['Take Away', 'Dine In', 'Delivery', 'Cash', 'Instapay']) {
...
new_emphasis = """  for (const name of ['Take Away', 'Delivery', 'Cash', 'Instapay']) {
    const control = cart.getByRole('button', { name, exact: true });
    await expect(control).toBeVisible();
    expect(await control.evaluate((node) => getComputedStyle(node).fontWeight)).toBe('700');
  }
...
if source.count(old_mobile) != 1:
    raise SystemExit(f'Expected one stale mobile payment block, found {source.count(old_mobile)}')
if source.count(old_emphasis) != 1:
    raise SystemExit(f'Expected one stale emphasis block, found {source.count(old_emphasis)}')

path.write_text(source.replace(old_mobile, new_mobile).replace(old_emphasis, new_emphasis))
