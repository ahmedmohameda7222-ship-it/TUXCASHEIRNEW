...
export async function collectCatalogArchitectureViolations(root) {
  ...
  if (!(await exists(path.join(contractRoot, 'package.json')))) {
    ...
    if (pkg.name !== '@tux/catalog-contracts') violations.push('catalog contract package name must be @tux/catalog-contracts');
  ...
  for (const filename of files.filter((candidate) => sourceExtension.test(candidate))) {
    ...
    if (rel.startsWith('packages/catalog-contracts/')) {
      if (/apps\/(?:operations|operations-desktop)|@tux\/operations|@supabase\/supabase-js|\.from\(/.test(source)) {
        violations.push(`catalog transport contract imports/uses application or persistence implementation: ${rel}`);
      }
    ...
    if ((rel.startsWith('apps/menu/') || rel.startsWith('apps/admin/')) && /apps\/(?:operations|operations-desktop)|@tux\/operations/.test(source)) {
      violations.push(`Menu/Admin imports Operations implementation: ${rel}`);
    }

    if ((rel.startsWith('apps/') || rel.startsWith('supabase/functions/catalog-public/')) && source.includes('SUPABASE_SERVICE_ROLE_KEY')) {
      violations.push(`privileged service-role secret referenced outside trusted catalog-admin/server boundary: ${rel}`);
    }
  ...
  if (await exists(publicFunction)) {
    ...
    if (source.includes('SUPABASE_SERVICE_ROLE_KEY')) violations.push('catalog-public must never use service-role credentials');
  }
  ...
  if (await exists(adminFunction)) {
    ...
    if (!source.includes('SUPABASE_SERVICE_ROLE_KEY')) violations.push('catalog-admin trusted adapter must own its server-only privileged credential explicitly');
  }
